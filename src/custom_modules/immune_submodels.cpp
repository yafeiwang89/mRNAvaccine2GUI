...
// NOT USED in mRNA vaccine project now 
...
void macrophage_phenotype( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	while( n < neighbors.size() )
	{
		...
		std::vector<double> displacement = pContactCell->position - pCell->position;
		double cell_cell_distance = norm( displacement ); 
...
void DC_phenotype( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	if( pCell->custom_data["activated_immune_cell"] > 0.5 ) // (Adrianne) activated DCs that don't leave the tissue can further activate CD8s increasing their proliferation rate and attachment rates
	{
		...
		while( n < neighbors.size() )
		{
			...
			// Find the euclidean distance between the DC and the cell it's testing
			std::vector<double> displacement = pTestCell->position - pCell->position;
			double cell_cell_distance = norm( displacement ); 
			...
			// Finding the G0G1 and S phase index and setting the transition rate to be non zero so that CD8 and CD4 T cells 
			// start proliferating after interacting with DC
			static int cycle_G0G1_index = flow_cytometry_separated_cycle_model.find_phase_index( PhysiCell_constants::G0G1_phase ); 
			static int cycle_S_index = flow_cytometry_separated_cycle_model.find_phase_index( PhysiCell_constants::S_phase ); 
			...
			{
				// (Adrianne) DC induced T cell attachement rate		
				pTestCell-> custom_data["cell_attachment_rate"] = parameters.doubles("DC_induced_CD8_attachment"); 
				...
				n = neighbors.size();  // maybe removed ???		
			}

			else if( pTestCell != pCell && pTestCell->phenotype.death.dead == false && pTestCell->type == CD4_Tcell_type 
				&& cell_cell_distance<=parameters.doubles("epsilon_distance")*(radius_DC+radius_test_cell))
			{
				pTestCell->phenotype.cycle.data.transition_rate(cycle_G0G1_index,cycle_S_index) = parameters.doubles("DC_induced_CD4_proliferation"); 			
	...
	{		
		...
		// In mRNA vaccine project, DCs become activated if the antigenic_epitope value is greater than 20 or ISF concentration is greater than 0.5

		static int ISF_index = microenvironment.find_density_index("ISF");
		static int epitope_index = pCell->custom_data.find_variable_index( "antigenic_epitope" ); 
		double ISF_conc = pCell->nearest_density_vector()[ISF_index];
		double antigenic_epitope = pCell->custom_data[epitope_index]; 

		if( ISF_conc > parameters.doubles("ISF_needed_for_DC_activation") || 
			antigenic_epitope > parameters.doubles("antigenic_epitope_needed_for_DC_activation")) 
		{		
...
void CD8_Tcell_phenotype( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	{	
		...
		// In mRNA vaccine project, all immune cells have no cycling rate when defined in config/.xml
...
// Refer cancer_immune_3D of sample project for details
// https://github.com/MathCancer/PhysiCell/blob/master/sample_projects/cancer_immune/custom_modules/cancer_immune_3D.cpp
...
void immune_submodels_setup( void )
{
	...
	// The function of CD8 in mRNA vaccine project is to attach with tumor cells and kill them 
		// set version info 
	...
	// set up macrophages. NOT USED in mRNA vaccine project actually 
	...
	// The function of DC in mRNA vaccine project is to recruit CD8 and CD4 
	// (through translation of LNP-mRNA to antigenic epitope)
	// and increase both proliferation rate if activated
	...
	// call the phenotype function in the DC_LNP_mRNA.cpp !!!!!
	...
	// The function of CD4 in mRNA vaccine project is to recruit antibody
	// set version info 
...
void immune_cell_recruitment_LNP( double dt )
{
	...
	static double CD8_recruit_rate = parameters.doubles( "CD8_recruit_rate" ); 
	static double CD4_recruit_rate = parameters.doubles( "CD4_recruit_rate" ); 
	static double Ig_recruit_rate = parameters.doubles( "Ig_recruit_rate" ); 

	static bool CD8_T_enabled = parameters.bools( "cytotoxic_T_response" );  // true;
	// if no antibody response, there is no CD4 recruited either
	static bool antibody_enabled = parameters.bools( "antibody_response" );  // true;


	// when T cells and antibody response activated
	static double activate_immune_response = 60*parameters.doubles( "activate_immune_response_time" );  	

	if ( PhysiCell_globals.current_time <= activate_immune_response - tolerance )
	{ return; }
	...
	if( t_immune > t_next_immune- tolerance )
	{
		...
		// #pragma omp parallel for  // parallel may arise issue
		...
		// std::cout << "\tantigen   " << total_antigenic_epitope << std::endl;  

		// expected number of  CD8 T
		// add stochasticity
		double number_of_new_cells_prob = UniformRandom()* CD8_recruit_rate* total_antigenic_epitope * elapsed_time; 
		...
		{
			number_of_new_cells_int = 2;   // try to avoid large number of CD8 T
		}
		*/
		
		if( number_of_new_cells_int && CD8_T_enabled )
		{
		...
		number_of_new_cells_prob = UniformRandom()* CD4_recruit_rate* total_antigenic_epitope *elapsed_time; // slow down , 0.01
		...
		if( number_of_new_cells_int>2 )
		{
			number_of_new_cells_int = 2;   // try to avoid large number of CD4 T
		}
		*/

		if( number_of_new_cells_int && antibody_enabled )
		{
		...
		// antibody recruitment in mRNA vaccine project
		...
		// std::cout << "\tCD4   " << total_CD4_Tcell << std::endl;  

		double number_of_new_Ig_prob = UniformRandom()* Ig_recruit_rate *total_CD4_Tcell *elapsed_time; // slow down 
		int number_of_new_Ig_int = floor(number_of_new_Ig_prob);
