...
void immune_cell_motility_direction( Cell* pCell, Phenotype& phenotype , double dt )
{
	...
	static int immunostimulatory_index = microenvironment.find_density_index( "ISF");
	static int debris_index = microenvironment.find_density_index( "debris");
	...
	std::vector<double> gradC = pCell->nearest_gradient(immunostimulatory_index);
	normalize( &gradC ); 
	gradC *= pCell->custom_data["sensitivity_to_immunostimulatory_chemotaxis"];
...
void DC_phenotype( Cell* pCell, Phenotype& phenotype, double dt )
{
	...
	else 
	{
		...
		double virus_amount = 1.0;  // meanless here, just test !!!!!!
...
void immune_cell_recruitment_LNP( double dt )
{
	...
	if( t_immune > t_next_immune- tolerance )
	{
		...
		number_of_new_cells_prob = UniformRandom()*total_antigenic_epitope * elapsed_time;
